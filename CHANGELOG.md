...
- README now follows the epicsagas install standard (curl → irm → Homebrew → cargo-binstall → cargo install) with a matching Updating table.
- `readme = "README.md"` package metadata (required for crates.io publish).
- **MCP stdio server** (`research serve`): exposes research-agent as 11 MCP tools (init, ingest, index_rebuild, query_papers, analyze_gaps, list_gaps, generate_report, topics_list, topic_add, state, update_read) so an LLM agent can drive the research flow directly. Adaptive dispatch: MCP is the primary interface for MCP hosts, CLI is the fallback for terminals/CI.
...
- README restructured to the epicsagas install standard (curl → irm → Homebrew → cargo-binstall → cargo install) with a matching Updating table and an MCP-server section.
