...
**[The Ontology Lineage](https://epicsagas.github.io/ontology-explorer/en/)** — 2300 лет исследований онтологий на одной статической странице (корейский · английский): хронология, интегрированная архитектура, обучающий маршрут из пяти этапов и обозреватель всех 425 статей — собраны, проанализированы и структурированы с помощью research-agent.

## Возможности

| | Возможность | Почему это важно |
|--|------------|------------------|
| 🤖 | MCP-сервер | 18 инструментов, которыми ваш агент управляет напрямую через stdio — без ключей API |
...
Плагин автоматически устанавливает бинарный файл `research` при старте сессии и предоставляет 18 инструментов MCP, позволяя агенту автономно собирать, искать, анализировать и сохранять отчеты с помощью собственной модели.
...
Как эти запросы выглядят на уровне инструментов:

| Вы просите | Агент выстраивает цепочку |
|---------|------------------|
| Полное исследование темы от начала до конца | `init` → `topic_add` → `ingest` (arXiv/S2, с привязкой к теме) → `topic_brief` → агент анализирует сводку собственной моделью → `gaps_record` → повторный `ingest` уже под найденные пробелы → `report_material` → `report_save` |
| Найти что-то из уже собранного | `query_papers` → `paper_body(id, query=...)` цитирует подходящий фрагмент с указанием раздела и страницы |
| Отслеживать прочитанное | `update_read` (статус, оценка 1-5); `state` для обзора покрытия |
| Открыть панель | Bash: `research dashboard`, затем сообщает, что http://127.0.0.1:7777 запущен |

Все операции идут против одной локальной SQLite-библиотеки (по умолчанию `~/.research/research.db`). Для библиотеки конкретного проекта один раз направьте сервер на другой файл в MCP-конфигурации хоста (`"args": ["mcp", "--db", "./project.research.db"]`, `--db` — глобальный флаг) либо пусть агент передает `--db` при вызовах через скилл/CLI.
...
**Инструменты** (18): `init` · `ingest` · `index_rebuild` · `import_papers` · `paper_body` · `paper_references` · `query_papers` · `topic_brief` · `gaps_record` · `list_gaps` · `report_material` · `report_save` · `topics_list` · `topic_add` · `state` · `update_read` ·
...
### Работа с панелью (дашбордом)

Это визуальная сторона той же базы данных. Запустите в терминале и откройте в браузере:

```bash
research dashboard        # затем откройте http://127.0.0.1:7777 (только локальный доступ)
```

| Экран | Что показывает |
|--------|---------------|
| Overview | Размер библиотеки, глубину чтения, воронку конвейера, покрытие по темам |
| Papers | Таблицу библиотеки; клик по строке открывает детали, встроенный читатель показывает текст с привязкой к страницам или сохраненный PDF, здесь же правятся статус чтения и оценка |
| Pipeline | На каком этапе находится каждая статья: от обнаружения до глубокого прочтения |
| History | Лента всех событий в обратном хронологическом порядке |
| Results | Пробелы в знаниях и сформированные отчеты |
| Config | Редактирование `[llm]`, пути к рабочему пространству и настроек привязки панели |

Типичный цикл: проверьте покрытие тем в Overview, посмотрите записанные пробелы в Results, вернитесь в терминал и направьте следующую загрузку на эти пробелы, затем следите за прогрессом чтения в Papers.
...
**Цикл исследования: собрать, найти пробелы, закрыть их**

```bash
research init                                          # только первый запуск: настраивает БД и LLM
research topics add "Graph DB internals"               # выводит идентификатор темы
research ingest "latch-free graph database" --topic <TOPIC_ID> --limit 20
research gaps --topic <TOPIC_ID>                       # что вскрылось при сборе
# следующую загрузку направьте на то, что указал анализ пробелов:
research ingest "MVCC snapshot isolation graph store" --topic <TOPIC_ID>
research status                                        # статьи, покрытие и пробелы по темам
```

`research gaps` и `research report` используют поставщика `[llm]`, настроенного при первичной настройке; без него возвращается заглушка вместо ошибки.

**Найти что-то из уже собранного**

```bash
research query "latch-free transaction" --evidence     # ищет даже в тексте статьи, показывает раздел и страницу
research read <PAPER_ID> --body                        # выводит сохраненный полный текст
research read <PAPER_ID> --status completed --rating 5
```

**Отдельная библиотека на каждый проект**
